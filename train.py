...
def main() -> None:  # noqa: C901, PLR0915
    ...
    checkpoint_dir = model_path.parent / f"checkpoints_{exp_timestamp}"
    print(f"Checkpoint directory: {checkpoint_dir}")  # noqa: T201
    ...
    __track_grad = args.track_grad.lower() in ("true", "1", "t", "y", "yes")
    __plot_grad = args.plot_grad.lower() in ("true", "1", "t", "y", "yes")
    __render_mode = args.render_mode.lower()
    __num_envs = args.parallel  # Number of parallel environments
    __model_path = args.pretrained_model if args.pretrained_model else None
    __resume_checkpoint = args.resume_checkpoint if args.resume_checkpoint else None
    __checkpoint_interval = args.checkpoint_interval
    __model_save_interval = args.model_save_interval

    if __render_mode not in ("rgb_array", "human"):
    ...
    agent = QLearningAgent(
        envs,
        config,
        visualization=GradientLossTracker(512, 196, graphs=__plot_grad) if __track_grad else None,
        logger_mode="",
        # logger_mode="wp",
    )

    # Load from checkpoint or pretrained model
    if __resume_checkpoint:
        print(f"Resuming training from checkpoint: {__resume_checkpoint}")  # noqa: T201
        agent.load_checkpoint(__resume_checkpoint)
    elif __model_path:
        print(f"Loading pretrained model: {__model_path}")  # noqa: T201
        with suppress(OSError, ValueError, RuntimeError):
            agent.load_model(str(__model_path))

    agent.policy_model.to(device)

    # Handle interruptions gracefully
    def save_model_and_exit(_signum, _frame) -> None:  # noqa: ANN001
        print("\nReceived interruption signal. Saving checkpoint and exiting...")  # noqa: T201
        agent.save_checkpoint(str(checkpoint_dir / "emergency_checkpoint.pth"))
        sys.exit(0)
    ...
    episode_rewards_tmp = np.zeros(__num_envs)
    episode_lengths_tmp = np.zeros(__num_envs, dtype=int)
    ...
    for episode in tqdm(range(n_episodes)):
        ...
        def process_obs(obs: np.ndarray) -> np.ndarray:
            ...
            if isinstance(envs, VectorEnv):
                return np.concatenate([obs[key].reshape(__num_envs, -1) for key in obs], axis=1)
        ...
        while True:
            ...
            if thisistrue:
                ...
                grad_and_loss = agent.train_short_memory(states, actions, rewards, next_states, terminated)
                episode_logs["train_time"] += time.time() - train_start
                episode_logs["grad_value"] = grad_and_loss["grad"]
                episode_logs["loss_value"] = grad_and_loss["loss"]
            ...
            episode_lengths_tmp += np.ones(__num_envs, dtype=int)
        ...
        if (episode + 1) % __checkpoint_interval == 0:  # Save detailed checkpoint every N episodes
    ...
    print("Training completed. Saving final checkpoint...")  # noqa: T201
    agent.save_periodic_checkpoint(n_episodes - 1, str(checkpoint_dir))
