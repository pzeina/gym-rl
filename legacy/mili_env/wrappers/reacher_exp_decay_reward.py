...
class ReacherRewardWrapper(gym.Wrapper):
    ...
    def _backpropagate_reward(self, reward: np.ndarray, *, show_heatmap: bool = False) -> None:
        ...
        for past_step in range(len(self.obs_history)):
            ...
            if steps_back <= self.history_length:
                ...
                # Hook for external agent memory systems could go here
                _ = obs, action, terminated  # placeholders to avoid unused warnings
    ...
    def _generate_heatmap(self) -> None:
        ...
        # Target no longer part of observation; optionally mark initial agent position
        if self.obs_history:
            min_pos_len = 2
            first_obs = self.obs_history[0]
            pos = first_obs.get("position") if isinstance(first_obs, dict) else None
            if pos is not None and len(pos) >= min_pos_len:
                p = np.round(np.asarray(pos)).astype(int)
                p[0] = np.clip(p[0], 0, env.width - 1)
                p[1] = np.clip(p[1], 0, env.height - 1)
                env_map[p[1], p[0]] = 1.0
