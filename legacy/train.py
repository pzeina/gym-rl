...
from mili_env.envs.reward_functions import register_default_rewards
...
class TerrainWorldRLlibWrapper(MultiAgentEnv):
    ...
    def __init__(self, config: dict[str, Any] | None = None) -> None:
        ...
        reward_function = env_config.get("reward_function", None)
    # auto_follow_orders support removed in env; keep config parsing for future use if needed
        ...
        terrain_env = cast("TerrainWorldEnv", self.env.unwrapped)
        register_default_rewards(terrain_env)

        # Set specific reward function if provided, otherwise check environment variable
        if reward_function:
            terrain_env.set_reward_function(reward_function)
        else:
            # Check environment variable for reward function
            env_reward_function = os.environ.get("REWARD_FUNCTION")
            if env_reward_function:
                terrain_env.set_reward_function(env_reward_function)

        # Communication actions are always enabled natively in the underlying env
        ...
        # Each agent observes: position(2) + distance(1) + direction(1) + energy(1) = 5 features
        self._obs_dim = 5
        ...
        # Only expose the movement part to policies (env handles comms always-on internally)
        move_spaces = {}
        for aid in self.agent_ids:
            sp = act_space.spaces[aid]
            if isinstance(sp, gym.spaces.Dict):
                # Prefer nested combat.move if available
                combat_sp = sp.spaces.get("combat")
                if isinstance(combat_sp, gym.spaces.Dict) and "move" in combat_sp.spaces:
                    move_spaces[aid] = combat_sp.spaces["move"]
                elif "move" in sp.spaces:
                    # Legacy: top-level move
                    move_spaces[aid] = sp.spaces["move"]
                else:
                    move_spaces[aid] = gym.spaces.Discrete(5)
            else:
                # Fallback to a basic Discrete of 5 actions
                move_spaces[aid] = gym.spaces.Discrete(5)
        self.action_space = move_spaces  # type: ignore[assignment]
        ...
        # Cache communication slots for action mapping
        self._comm_slots = int(getattr(terrain_env, "MAX_COMMS_PER_STEP", 3))
    ...
    def step(
        ...
        # Map policy actions (movement-only ints or dicts) to env actions with comm no-ops
        env_actions: dict[str, Any] = {}
        num_agents = self._num_agents
        for aid in self.agent_ids:
            a = action_dict.get(aid, 0)
            if isinstance(a, dict) and "move" in a:
                move_val = int(a.get("move", 0))
            else:
                # Accept ints and numpy scalars; anything else defaults to IDLE
                move_val = int(a) if isinstance(a, (int, np.integer)) else 0
            # Prefer nested schema used by the environment: combat + comm
            env_actions[aid] = {
                "combat": {"move": move_val, "fire_enemy": num_agents},
                "comm": {
                    "types": [0] * self._comm_slots,
                    "targets": [num_agents] * self._comm_slots,
                },
            }
...
def main() -> None:
    ...
    try:
        ...
        env_config = {
            ...
            "render_mode": None,
            "auto_follow_orders": bool(getattr(args, "auto_follow_orders", False)),
        }
