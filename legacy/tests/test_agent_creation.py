...
class TestAgentCreation(unittest.TestCase):
    ...
    def test_agent_target_assignment(self):
        """Deprecated: targets removed; validate agent identities and teams instead."""
        agent_ids = [agent.agent_id for agent in self.env.agents]
        # Agent IDs should be unique and sequential starting at 0
        self.assertEqual(sorted(agent_ids), list(range(len(self.env.agents))))
        # Validate team assignment presence
        for agent in self.env.agents:
            self.assertTrue(hasattr(agent, "team_id"))
