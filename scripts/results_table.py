...
def table() -> str:
    ...
    stamp = (f"\nAll {len(members)} runs trained from commit `{commit[:8]}` on the shipped "
             "reward defaults, no `--reward` overrides. Generated by "
             "`scripts/results_table.py`; `tests/test_results_table.py` fails if this "
             "table and the runs disagree." if commit else
