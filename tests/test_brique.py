...
def test_spaces_frozen_under_brique():
    ...
    assert N_ACTIONS == 231  # degraded-comms cycle: 228 + 3 appended
    assert OBS_DIM == 328  # degraded-comms cycle: 220 + 94 acoustic + 14 cohesion
    env = make_env(_spec(n_traps=3))
    obs, _ = env.reset(seed=0)
    assert env.action_space(env.possible_agents[0]).n == 231
    for a in env.agents:
        assert obs[a]["observation"].shape == (328,)
        assert obs[a]["action_mask"].shape == (231,)
