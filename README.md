...
|---|---|---|---|---|---|---|---|---|---|
| `fireteam` | `fireteam_v14` | 0.93 ± 0.05 (N=100) | 0.98 ± 0.03 (N=100) | 4.7 pt | 17% | 1% | 93/93 | 88% | pass |
| `fireteam_defend` | `fireteam_defend_v25` | 1.00 ± 0.00 (N=100) | 0.99 ± 0.02 (N=100) | 1.5 pt | 4% | 0% | 100/100 | 100% (sitrep, 0.027/sitrep) | pass |
| `squad` | `squad_ctrl_v2_seed12` | 0.92 ± 0.05 (N=100) | 0.95 ± 0.04 (N=100) | 6.2 pt | 35% | 1% | 92/92 | 88% | pass |
| `squad_recon` | `squad_recon_v13` | 0.98 ± 0.03 (N=100) | 1.00 ± 0.00 (N=100) | 0.9 pt | 18% | 0% | 98/98 | 94% | pass |
| `squad_screen` | `squad_screen_v19` | 1.00 ± 0.00 (N=100) | 1.00 ± 0.00 (N=100) | 0.4 pt | 7% | 0% | 100/100 | 94% | pass |
| `patrol_brique` | `patrol_brique_v44_seed19` | 0.98 ± 0.03 (N=100) | 0.97 ± 0.03 (N=100) | 0.9 pt | 3% | 0% | 98/98 | 84% | pass |
| `defend_brique` | `defend_brique_v19` | 1.00 ± 0.00 (N=100) | 0.96 ± 0.04 (N=100) | 0.5 pt | 8% | 0% | 100/100 | 99% (sitrep, 0.053/sitrep) | pass |
| `platoon` | `platoon_v14_seed13` | 1.00 ± 0.00 (N=100) | 0.99 ± 0.02 (N=100) | 0.8 pt | 12% | 0% | 100/100 | 94% | pass |
| `platoon_hard` | `platoon_hard_v7_seed13` | 0.93 ± 0.05 (N=100) | 0.96 ± 0.04 (N=100) | 7.4 pt | 29% | 1% | 93/93 | 0% | pass |

8 of 9 runs trained against the same `cohort/` tree (`ee378f01`) on the shipped reward defaults, no `--reward` overrides. `squad` (`squad_ctrl_v2_seed12`) trained on `81a9cec6` — a disclosed exception (assistant, disclosed for the owner decision 2026-08-26): squad_ctrl_v2_seed12 trained at commit 7247122 (tree 81a9cec6); the other eight members trained during the v1.23 campaign at tree ee378f01. The delta is commits 297cf35 and de1f53d — the comm-model marker waiver and the checkpoint-selection change that reads it. BOTH ARE INERT FOR THIS MEMBER: COMM_MODEL_MARKER_WAIVERS has a single entry, keyed 'jammed', and `squad` is comm_model='global', so `report_gate_waived` resolves False and best_save_gate takes exactly the pre-change branch. The training path is therefore identical, and only the recorded tree differs. ROOT CAUSE, recorded because it was a planning error and not bad luck: scripts/campaigns/v1_23_fleet.jobs deliberately carried NO squad job, on the reasoning that squad_ctrl_v2_seed12 was bit-identical in CONFIG to one. That was true and beside the point — it was trained before the campaign froze the tree, so it is not on the frozen tree. A squad job in the campaign would have cost 45 minutes and avoided this. The clean fix is a squad retrain on the fleet tree; it is NOT taken now because the price-dispersion cycle retrains every member imminently, which resolves it at no extra cost. Generated by `scripts/results_table.py`; `tests/test_results_table.py` fails if this table and the runs disagree.

`(sitrep, …)` in root-reported: a DEFEND/DENY root cannot file a DONE claim — doctrine masks MISSION COMPLETE shut on a continuous posture — so the SITREP route is that root's only counted way to close the operation, and the rate measures it. The number beside it is closes per root SITREP emitted: low means the closes were bought with report volume rather than timing.
<!-- BASELINE-TABLE:END -->
