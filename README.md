...
|---|---|---|---|---|---|---|---|---|---|
| `fireteam` | `fireteam_v12` | 0.94 ± 0.05 (N=100) | 0.96 ± 0.04 (N=100) | 4.6 pt | 21% | 0% | 94/94 | 91% | pass |
| `fireteam_defend` | `fireteam_defend_v23` | 0.99 ± 0.02 (N=100) | 0.99 ± 0.02 (N=100) | 0.9 pt | 4% | 0% | 99/99 | 100% (sitrep, 0.035/sitrep) | pass |
| `squad` | `squad_v27` | 0.97 ± 0.03 (N=100) | 0.97 ± 0.03 (N=100) | 1.2 pt | 12% | 1% | 97/97 | 96% | pass |
| `squad_recon` | `squad_recon_v11` | 1.00 ± 0.00 (N=100) | 0.99 ± 0.02 (N=100) | 0.3 pt | 11% | 0% | 100/100 | 99% | pass |
| `squad_screen` | `squad_screen_v14` | 1.00 ± 0.00 (N=100) | 0.97 ± 0.03 (N=100) | 0.5 pt | 18% | 0% | 100/100 | 95% | pass |
| `patrol_brique` | `patrol_brique_v41_seed14` | 0.98 ± 0.03 (N=100) | 0.96 ± 0.04 (N=100) | 2.5 pt | 2% | 2% | 98/98 | 88% | pass |
| `defend_brique` | `defend_brique_v17` | 1.00 ± 0.00 (N=100) | 0.89 ± 0.06 (N=100) | 0.2 pt | 2% | 0% | 100/100 | 100% (sitrep, 0.053/sitrep) | pass |
| `platoon` | `platoon_v8` | 1.00 ± 0.00 (N=100) | 1.00 ± 0.00 (N=100) | 0.7 pt | 17% | 0% | 100/100 | 93% | pass |
| `platoon_hard` | `platoon_hard_v5_seed12` | 0.94 ± 0.05 (N=100) | 0.91 ± 0.06 (N=100) | 8.2 pt | 28% | 1% | 94/94 | 0% | **FAIL: closed_on_root_report_rate** (shipped by owner decision 2026-08-21) |

8 of 9 runs trained against the same `cohort/` tree (`becc3ec4`) on the shipped reward defaults, no `--reward` overrides. `platoon_hard` (`platoon_hard_v5_seed12`) trained on `079f78f0` — a disclosed exception (owner decision 2026-08-21): trained one commit ahead of the fleet tree (cde45ab8, which IS this scenario's shipped semantics: the idle-time price as ScenarioSpec.reward_overrides). No other member's environment moved; retraining the fleet onto this tree was mooted the same day by the acoustics spaces break. Pinned so any further movement is drift. Generated by `scripts/results_table.py`; `tests/test_results_table.py` fails if this table and the runs disagree.
